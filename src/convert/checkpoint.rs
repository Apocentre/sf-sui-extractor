...
use super::common::{
  convert_gas_cost_summary, convert_checkpoint_commitment
};

fn convert_end_of_epoch_data(source: &EndOfEpochData) -> pb::EndOfEpochData {
  pb::EndOfEpochData {
    next_epoch_committee: todo!(),
    next_epoch_protocol_version: todo!(),
    epoch_commitments: source.epoch_commitments.iter().map(convert_checkpoint_commitment),
  }
}
